...
export default function App() {
  ...
  useEffect(() => {
    ...
    const checkHealth = async () => {
      try {
        ...
        if (isMounted) setSystemStatus('down');
      }
    };
    checkHealth();
  ...
  return (
    <div className="flex h-screen w-full bg-[#020617] text-slate-300 font-sans overflow-hidden">
      ...
      {isMobileMenuOpen && (
        <div 
          className="fixed inset-0 bg-[#020617]/80 backdrop-blur-sm z-40 md:hidden" 
          onClick={() => setIsMobileMenuOpen(false)} 
        />
      )}
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} border-r border-white/5 bg-[#020617] flex flex-col`}>
              ...
              >
                {isEditing ? (
                  ...
                  <>
                    <button
                      onClick={() => { 
                        setActiveEndpointId(ep.endpointId); 
                        setActiveView('stream'); 
                        setIsMobileMenuOpen(false);
                      }}
          ...
          <div className="mt-4 px-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="relative flex h-2.5 w-2.5">
                ...
                <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${systemStatus === 'operational' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]' : systemStatus === 'checking' ? 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.8)]' : 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]'}`}></span>
              </span>
              <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">
                {systemStatus === 'operational' ? 'All Systems Operational' : systemStatus === 'checking' ? 'Checking Status...' : 'System Degraded'}
              </span>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#020617]">
        {/* Top Header */}
        <header className="h-16 border-b border-white/5 bg-[#020617] flex items-center justify-between px-4 sm:px-6 z-10 sticky top-0">
          <div className="flex items-center gap-4 flex-1">
            <button 
              className="md:hidden p-2 text-slate-400 hover:text-white hover:bg-[#0f172a]/50 rounded-lg"
